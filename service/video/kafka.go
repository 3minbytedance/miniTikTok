...
// video 服务专属的视频发布消息与生产/消费者（api 网关不直接访问 kafka 业务 topic）。
...
func InitVideoKafka() {
	...
	VideoMQInstance.Producer = kafkamw.NewProducer(VideoMQInstance.Topic)
	VideoMQInstance.Consumer = kafkamw.NewConsumer(VideoMQInstance.Topic, VideoMQInstance.GroupId)
...
func ensureTopic(topic string) {
	client := &kafka.Client{Addr: kafka.TCP(kafkamw.Brokers()...)}
...
func (m *VideoMQ) Produce(message *VideoMessage) error {
	return kafkamw.ProduceMessage(m.Producer, message)
}
