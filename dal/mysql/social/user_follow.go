...
func DeleteFollowById(userId, followId uint) error {
	return DB.Where("user_id = ? AND follow_id = ?", userId, followId).
...
// IsFollowing 判断关注关系是否存在；只需存在性，用 Limit(1) 避免 COUNT 全量扫描。
func IsFollowing(userA uint, userB uint) bool {
	var id uint
	return DB.Model(&model.UserFollow{}).
		Select("id").
		Where("user_id = ? AND follow_id = ?", userA, userB).
		Limit(1).
		Find(&id).RowsAffected > 0
}
